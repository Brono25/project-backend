...
beforeEach(() => {
  ...
  token2 = authRegisterV1(...args).token;
  // Channels 0,1,2
  ...
  // Error cases
  invalidUserId = Math.abs(userId0) + Math.abs(userId1) + Math.abs(userId2) + 10;
  invalidToken = Math.abs(token0) + Math.abs(token1) + Math.abs(token2) + 10;
